...
class User(db.Model, UserMixin):
    ...
    def following_posts(self):
        ...
        return (sa.select(Post).join(Post.author.of_type(Author)).join(Author.followers.of_type(Follower), isouter=True).where(sa.or_(Follower.id == self.id, Author.id == self.id)).group_by(Post).order_by(Post.timestamp.desc()))
...
class Post(db.Model):
    ...
    user_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey(User.id), index=True) #Crea la referencia a la tabla
    author: so.Mapped[User] = so.relationship(back_populates="posts") #Crea la relacion con la otra tabla mediante un valor 
